...
```bash
!`if [ -f vitest.config.ts ] || [ -f vitest.config.js ] || [ -f vitest.config.mts ]; then echo "Test runner: Vitest"; elif [ -f jest.config.ts ] || [ -f jest.config.js ] || [ -f jest.config.cjs ]; then echo "Test runner: Jest"; else echo "Test runner: unknown — ask the user"; fi`
```
...
1. **Sketch behaviors** — Before writing any code, list the behaviors to implement as placeholder tests (e.g. `it.todo(...)`).
...
- **Vitest**: See `references/vitest-examples.md` for test modifiers (`it.todo`, `it.skip`, `it.fails`, etc.) and the `--changed` flag.
- **Jest**: `npx jest --changedSince=HEAD` or `npx jest <test-file>`.
